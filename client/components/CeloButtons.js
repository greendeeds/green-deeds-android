...
export default class CeloButtons extends React.Component {
  ...
  donateToFaucet = async () => {
    ...
    let celotx = await goldToken
      .transfer(faucetAddress, amount)
      .send({ from: donatingAccount })
      .catch((err) => console.log("celotx ERR: ", err));
    let cUSDtx = await stableToken
      .transfer(faucetAddress, amount)
      .send({ from: donatingAccount, feeCurrency: stableToken.address })
      .catch((err) => console.log("cUSDtx ERR: ", err));

    console.log("got txs");

    // TODO: Error unknown account
    let celoReceipt = await celotx
      .waitReceipt()
      .catch((err) => console.log("celoReceipt ERR: ", err));
    let cUSDReceipt = await cUSDtx
      .waitReceipt()
      .catch((err) => console.log("cusdReceipt ERR: ", err));
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <Button title="Get Faucet Info" onPress={() => this.getFaucetInfo()} />
        ...
        <Button title="Get cGLD" onPress={() => this.getcGLD()} />
        <Button title="Get cUSD" onPress={() => this.getcUSD()} />
